...
if (isset($_POST['title']) && isset($_POST['topic']) && isset($_POST['description']) && isset($_POST['image']) && !empty($_POST['title']) && !empty($_POST['topic']) && !empty($_POST['description']) && !empty($_POST['image'])) {
    ...
    $sql = $db->prepare("UPDATE `post` SET `title` = :title, `topic` = :topic, `description` = :description, `image` = :image  WHERE `post`.`id` = $post_id");
